...
## 2026-07-13 post-`0eb51e5` reconciliation

- Added bounded `POST /v1/rules/preview`: evaluates at most 1,000 persisted rules without mutation, returns the resulting job request, identifies every matching rule, and distinguishes selected from shadowed destination/speed-limit actions. A deterministic test proves preview output matches actual rule application.
- Registered the rule-preview operation in the generated OpenAPI operation inventory.
- rqbit response decoding now streams into a hard 4 MiB ceiling even when `Content-Length` is absent or dishonest. This closes the remaining unbounded allocation shared by DHT, diagnostics, and stable torrent JSON endpoints.
- DHT statistics now use a Ravyn-owned typed contract matching rqbit's documented `id`, outstanding-request, and IPv4/IPv6 routing-size fields. Routing-table responses use a stable `v4`/`v6` envelope and reject more than 16,384 JSON nodes, nesting beyond 32 levels, strings beyond 4 KiB, or missing families. Error bodies are independently capped at 8 KiB, and rqbit 404 responses become explicit disabled/unsupported DHT capability errors.
- Targeted rule tests (5/5) and `cargo check --locked --all-targets` pass after these changes. This is tranche evidence, not a final completion gate.
- `max_active` is now an immediate live setting backed by an atomic concurrency gate rather than a fixed Tokio semaphore. Decreases never cancel active work and prevent new claims until usage falls below the limit; increases admit work immediately. A deterministic permit test covers decrease, drain, and increase behavior. API middleware settings remain honestly classified as restart-required until their middleware-owned state is made shareable with the settings handler.
- API middleware state is now shared with the settings handler. Request timeout, maximum concurrency, per-client requests/minute, and burst capacity are applied live together; existing token buckets are cleared so no client retains stale refill/burst parameters. The health timeout remains capped at five seconds, active requests are not cancelled by a concurrency decrease, reset restores the startup baseline, and the settings response classifies all four fields as live.
- Media process parsing now rejects any single yt-dlp machine-output line above 1 MiB without first allocating an unbounded `String`; CRLF/LF normalization is deterministic. The exported per-job archive snapshot is owned by an RAII guard, so cancellation, timeout, parse failure, spawn/pipe errors, and normal completion all remove it. Targeted tests cover the hostile line and every-exit cleanup primitive.
